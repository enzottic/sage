...
final class RecurringReminderCoordinator {
    ...
    func start(configuration: AppConfiguration) {
        ...
        if observers.isEmpty {
            for name in [ModelContext.didSave, UIApplication.didBecomeActiveNotification,
    ...
    func refresh() {
        ...
        dailyScheduler.refresh(enabled: configuration.dailyExpenseReminderEnabled,
                               timeMinutes: configuration.dailyExpenseReminderTimeMinutes)
        scheduler.refresh(enabled: enabled, hideDetails: hideDetails) { [container] in
