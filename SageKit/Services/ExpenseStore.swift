...
public class ExpenseStore {
    ...
    public func fetchExpenses(for month: Date) -> [Expense] {
    ...
    public func fetchAllExpenses() -> [Expense] {
        let fetchDescriptor = FetchDescriptor<Expense>(
            sortBy: [SortDescriptor(\Expense.date, order: .reverse)]
        )
        return (try? context.fetch(fetchDescriptor)) ?? []
    ...
    public func fetchTag(id: UUID) -> ExpenseTag? {
    ...
    public func monthlyTotal(for month: Date = .now) -> Double {
        fetchExpenses(for: month).total
    }

    public func monthlyTotal(category: ExpenseCategory, month: Date = .now) -> Double {
        fetchExpenses(for: month).filter { $0.category == category }.total
    }

    public func monthlyTotal(tagId: UUID, month: Date = .now) -> Double {
    ...
    public func remainingBudget(for category: ExpenseCategory, month: Date = .now) -> Double {
        budget(for: category) - monthlyTotal(category: category, month: month)
    }

    public func totalRemainingBudget(month: Date = .now) -> Double {
